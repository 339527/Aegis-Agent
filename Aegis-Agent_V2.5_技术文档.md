...
Aegis-Agent 是一个测试开发项目。项目以若依（RuoYi）后台系统为被测对象，完成接口自动化测试框架搭建，并扩展 AI Agent 安全测试原型，用于验证 Prompt 注入、命令注入、越权工具调用和敏感信息泄露等风险。
...
| XSS | `<script>` |
| SQL 注入 | `select`、`drop`、`union`、条件拼接 |
| 敏感信息 | `zhipu_api_key`、`密钥`、`密码` |
