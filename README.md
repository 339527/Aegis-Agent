...
```text
ry/
├── ai_core/                 # 🧠 AI 大脑核心区 (底层基座、审查官、执行特工)
│   └── agents.py            
├── api/                     # 🦾 业务工具箱 (Agent 的手脚，纯粹的接口定义)
│   ├── auth_api.py          # 鉴权模块
│   └── user_api.py          # 用户管理模块
├── common/                  # ⚙️ 基础生态设施
│   ├── base_api.py          # HTTP 发包中枢与环境挡板拦截器
│   ├── mysql_util.py        # MySQL 直连探针 (带 CI 虚拟化)
│   └── redis_util.py        # Redis 破壁工具 (带 CI 虚拟化)
├── config/                  # 🎛️ 全局配置中心 (URL、账密、数据库配置)
├── data/                    # 📦 YAML 弹药库 (数据驱动载荷)
├── reports/                 # 📊 Allure 战报生成区
├── tests/                   # 🎬 自动化指挥部
│   ├── conftest.py          # 全局拦截器与 Token 注入器
│   ├── test_ruoyi_login.py  # 基础门禁连通性测试
│   ├── test_user_add_ddt.py # 传统 YAML 数据驱动测试
│   └── test_user_lifecycle.py # 核心：Agent 智能流转与恶意注入演练
├── .env                     # 🔐 隐形保险箱 (存放 ZHIPU_API_KEY，勿提交)
├── pytest.ini               # Pytest 引擎配置文件
└── run.py                   # 🚀 一键启动与战报编译总闸# 🛡️ RuoYi AI-Agent 自动化攻防测试框架 (V1.4 满血版)
