...
class RedisUtil:
    def __init__(self, host='localhost', port=6379, password='', db=0):
        ...
        if os.getenv("RUN_ENV") == "ci":
            logger.info("☁️ [企业级挡板] CI环境检测到：已拦截真实 Redis 连接，启用 Mock 对象")
            self.r = MagicMock()
        ...
        try:
            ...
            # ping 一下，测试是否真的连上了
            self.r.ping()
            logger.info(f"🟢 Redis 缓存直连成功 [{host}:{port}]")
        except Exception as e:
            logger.error(f"🔴 Redis 连接致命失败，请检查本地 Redis 是否启动: {e}")
            raise e
    ...
    def get_captcha_code(self, uuid):
        ...
        if code:
            ...
            logger.warning(f"⚠️ 未能在 Redis 中找到 key: {key} 的验证码！")
            return None
