...
@patch('ai_core.agents.SecurityAuditor.audit_payload')
async def test_concurrent_stress_with_async_sleep(mock_audit_payload, mock_parse_intention):
    """验证高并发请求下，Dispatcher 是否能保持稳定（带异步休眠）"""
    # Mock AI意图解析，直接返回工具调用结果
    mock_parse_intention.return_value = ("async_sleep_tool", {"sleep_time": 1})
    # Mock 安全审计，直接返回安全结果
    mock_audit_payload.return_value = {"is_safe": True, "risk_analysis": "[PASS] 安全"}
    ...
    assert mock_audit_payload.call_count == 5
